...
なお、バリデーションと OpenAPI 定義の zod への一本化（issue #83）は完了しています。
リクエストの検証と spec が同じスキーマから導かれるようになったため、
「zod スキーマと spec が食い違う」経路だけが無くなりました。契約テスト自体の役割は
変わっておらず、リクエスト契約は `express-openapi-validator`、レスポンス契約は
`tests/helpers/responseValidator.ts` が引き続き検証します。spec と実ルートの一致は
`openapi:check-routes`（CI では別ステップ）の担当で、上記の網羅性判定は
その保証を前提にしています。
